...
void point_update(int node, int lo, int hi, int idx, int val)
{
    if (lo == hi) // Leaf Node
    {
        if (lo == idx) // Update only at arr[idx]
            tree[node] = val;
    }
    else
    {
        ...
        if (idx <= mid)
            point_update(node*2, lo, mid, idx, val);
        else
            point_update(node*2+1, mid+1, hi, idx, val);
...
int range_query(int node, int lo, int hi, int i, int j)
{
    ...
    else // Partial overlap
    {
        ...
        int left  = range_query(node*2,      lo, mid, i, j);
        int right = range_query(node*2+1, mid+1,  hi, i, j);
    ...
    for (int test = 0; test < tests; ++test)
    {
        ...
        while (queries--)
        {
            ...
            else if (op == 'B')
            {
                int j, k;
                std::scanf("%d %d", &j, &k);
                point_update(1, 0, n-1, j, k);
            }
            else
            {
                int i, j;
                std::scanf("%d %d", &i, &j);
                int res = std::abs(m - range_query(1, 0, n-1, i, j));
