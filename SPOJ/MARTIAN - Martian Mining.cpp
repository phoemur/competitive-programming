...
// Recurrence relation
int solve_recursive(const std::vector<std::vector<int>>& Y, 
                    ...
                    int j)
{
    ...
    else
    {
        ...
        return std::max(sum_b + solve_recursive(Y, B, i, j-1), 
                        sum_y + solve_recursive(Y, B, i-1, j));
    }
}

// Dynamic programming bottom-up approach
int solve_bottomup(const std::vector<std::vector<int>>& Y, 
                   const std::vector<std::vector<int>>& B)
{
    assert(Y.size() == B.size());
    assert(Y[0].size() == B[0].size());
    
    const int n = Y.size(), m = Y[0].size();
...
int solve(const std::vector<std::vector<int>>& Y, 
          const std::vector<std::vector<int>>& B)
{    
    const int n = Y.size(), m = Y[0].size();
...
int main()
{
    ...
    for (int n, m; std::cin >> n >> m && (n != 0 || m != 0);)
    {
        std::vector<std::vector<int>> Y (n, std::vector<int>(m));
        std::vector<std::vector<int>> B (n, std::vector<int>(m));
        
        for (auto& row: Y)
            std::copy_n(std::istream_iterator<int>(std::cin), m, std::begin(row));
        
        for (auto& row: B)
            std::copy_n(std::istream_iterator<int>(std::cin), m, std::begin(row));

        // std::cout << solve_recursive(Y, B, n, m) << std::endl;
        // std::cout << solve_bottomup(Y, B) << std::endl;
        std::cout << solve(Y, B) << std::endl;
    }
