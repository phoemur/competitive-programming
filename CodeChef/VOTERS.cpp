...
int main()
{
    ...
    std::vector<int> first (n1 + n2 + n3);
    std::vector<int> result;
    
    std::copy_n(std::istream_iterator<int>(std::cin), n1+n2+n3, std::begin(first));
    ...
    while (true)
    {
        it = std::adjacent_find(it, first.end());
        
        if (it == first.end()) break;
        
        result.push_back(*it);
                
        while (it+1 != first.end() && *it == *(it+1))
            it = it + 1;
    }
