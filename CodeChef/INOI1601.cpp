...
{
    long ans = curr_max - wealth[node];
    
    for (int c : adj[node])
        if (wealth[c] > curr_max)
            ans = std::max(ans, DFS(c, wealth[c], adj, wealth));
        else
            ans = std::max(ans, DFS(c, curr_max, adj, wealth));
...
int main()
{
    ...
    std::printf("%ld\n", DFS(root, wealth[root], adj, wealth));
