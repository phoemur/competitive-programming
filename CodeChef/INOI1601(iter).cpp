...
long DFS(int root, 
         ...
         const std::vector<long>& wealth)
{
    
    std::stack<std::pair<int,long>> ST; // pair[node, curr_max]
    long ans = 0;
    ST.emplace(root, wealth[root]);
    ...
    while (!ST.empty())
    {
        ...
        std::tie(node, curr_max) = ST.top();
        ST.pop();
        
        ans = std::max(ans, curr_max - wealth[node]);
        
        for (int c : adj[node])
            if (wealth[c] > curr_max)
                ST.emplace(c, wealth[c]);
            else
                ST.emplace(c, curr_max);
    }
...
int main()
{
    ...
    std::vector<long> wealth (n);
    
    for (int i = 0; i < n; ++i)
        std::scanf("%ld", &wealth[i]);
